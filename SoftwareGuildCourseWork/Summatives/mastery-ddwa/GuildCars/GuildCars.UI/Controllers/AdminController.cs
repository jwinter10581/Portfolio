...
namespace GuildCars.UI.Controllers
{
    public class AdminController : Controller
    {
        ...
        public ActionResult AddUser(EmployeeAddViewModel model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    userMgr.Create(user, model.Employee.Password);
        ...
        [Authorize(Roles = "admin")]
        [ValidateAntiForgeryToken]
        public ActionResult EditUser(EmployeeEditViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    var userMgr = HttpContext.GetOwinContext().GetUserManager<UserManager<AppUser>>();
                    var oldUser = userMgr.FindByEmail(oldEmployee.EmployeeEmail);

                    if (userMgr.FindByEmail(oldEmployee.EmployeeEmail) != null)
                    {
                        userMgr.Delete(oldUser);
                    }
                    ...
                    if (model.Employee.Password is null)
                    {
                        userMgr.Create(newUser, oldEmployee.Password);
                        userMgr.AddToRole(newUser.Id, model.Employee.RoleName.ToLower());
                        model.Employee.Password = oldEmployee.Password;
                    }
                    else
                    {
                        userMgr.Create(newUser, model.Employee.Password);
                        userMgr.AddToRole(newUser.Id, model.Employee.RoleName.ToLower());
                    }
