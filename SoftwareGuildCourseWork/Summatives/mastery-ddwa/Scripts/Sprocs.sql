...
BEGIN
	SELECT EmployeeId, RoleName, EmployeeFirstName, EmployeeLastName, EmployeeEmail, [Password]
	FROM Employee e
...
AS
BEGIN
	SELECT EmployeeId, RoleName, EmployeeFirstName, EmployeeLastName, EmployeeEmail, e.Password
	FROM Employee e
		INNER JOIN Role r ON e.RoleId = r.RoleId
...
BEGIN
	INSERT INTO Employee(RoleId, EmployeeFirstName, EmployeeLastName, EmployeeEmail, [Password])
	SELECT (SELECT RoleId FROM [Role] r WHERE r.RoleName = @RoleName), @EmployeeFirstName, @EmployeeLastName, @EmployeeEmail, @Password
...
BEGIN
	...
	FROM Vehicle v
		...
		INNER JOIN Make mk ON mk.MakeId = md.MakeId
		INNER JOIN VehicleType vt ON v.VehicleTypeId = vt.VehicleTypeId
	WHERE vt.VehicleTypeName = @VehicleTypeName
